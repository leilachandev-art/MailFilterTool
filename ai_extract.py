...
def extract_line_items_from_pdf(pdf_bytes, field_defs):
    ...
    if container_field:
        grouping_instruction = (
            ...
            f"极其重要——不要把「有几行费用」和「有几个 container」搞混，这是最容易出错的地方：一份账单\n"
            f"哪怕列了好几笔不同名目的费用（比如运费、操作费、文件费、报关费分开各占一行，也就是「多项\n"
            f"服务的收费」），只要这些费用行对应的 container 编号（或者引用号栏位的值）全都是同一个，那\n"
            f"就还是只有一个 container，应该把这些费用金额加总，输出成一条记录，不要因为看到表格里有\n"
            f"好几行就拆成好几条——「有没有逐笔费用明细表」不能作为判断依据，「这些费用行对应的 container\n"
            f"编号是不是真的不一样」才是唯一的判断依据。具体做法：先把每一笔费用行对应的 container 编号\n"
            f"（或引用号）都摘出来看一遍，数一下里面有几个不同的值——只有一个不同值（不管这个值在表格里\n"
