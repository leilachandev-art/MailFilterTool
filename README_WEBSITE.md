...
2. 填筛选条件（关键词 / 精确匹配 / 信任发件人等）→ 保存配置
3. 投递方式选"打包下载 ZIP"（目前唯一可用的方式，OneDrive 那个选项还在调试，点了会提示暂未开放）
4. 点"立即运行"，下面会实时显示进度，跑完点"下载本次结果（ZIP）"，附件就下载到自己电脑了
...
- **OneDrive 同步暂未开放**：代码已经写好，Azure 应用注册那边还没调通，目前投递方式只支持"打包下载 ZIP"。
